...
class CustomLogisticRegression:
    ...
    def __init__(self, fit_intercept: bool = True, l_rate: float = 0.01, n_epoch: int = 100):
        ...
        self.coef_ = []
        self.mse_error_first = []  # mean square error on the first epoch of learning (EOL)
        self.mse_error_last = []  # mse on the last EOL
        self.log_loss_error_first = []  # log-loss error function on the 1-st EOL
        self.log_loss_error_last = []  # log-loss error function on last EOL

    def sigmoid(self, t) -> float:
        return 1 / (1 + math.exp(-t))

    def predict_proba(self, row: list, coef_: list) -> float:
        t = np.dot(row, coef_)
...
mse_error_first = clr.mse_error_first
mse_error_last = clr.mse_error_last
